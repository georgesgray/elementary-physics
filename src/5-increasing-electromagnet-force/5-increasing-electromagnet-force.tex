...
\begin{subequations}
\begin{align}
F(t) &= \frac{A_F}{A_R} \, R \, i^2(t) \\
P(t) &= V \, i(t)
\end{align}
\end{subequations}

and so the force to power ratio is constant over time:

\begin{equation}
\frac{F(t)}{P(t)} = \frac{\frac{A_F}{A_R} \, R \, i^2(t)}{V \, i(t)} = \frac{A_F}{A_R}
\end{equation}

This is true as long as $g$ in $A_F$ is constant.
If the force from the electromagnet is to be of any use it has to accelerate some mass.
In that case $g$ change and the force to power ratio is no longer constant.
